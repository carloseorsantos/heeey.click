...
8. **Convite por e-mail só vale para e-mail comprovado.** Em produção, "Confirm email" está desligado (`mailer_autoconfirm: true`). Assim, qualquer pessoa cria pela API pública do Supabase Auth uma conta com senha, já confirmada, usando o e-mail de outra. Como o app só entra por magic link e essas contas não têm senha, convites por e-mail só se ligam a contas confirmadas e sem senha (`proven_email()`). Ligar "Confirm email" no Supabase fecha também a ocupação de e-mails por cadastro com senha.
