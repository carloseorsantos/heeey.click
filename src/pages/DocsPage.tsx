...
export function DocsPage({
  ...
  // Scroll to top whenever currentSlug changes
  useEffect(() => {
                  ...
                  <nav className="space-y-1 max-h-[calc(100vh-10rem)] overflow-y-auto text-xs pr-2">
                    {toc.map((item) => (
                      <a
                        ...
                        className={`
                          block py-1 hover:text-brand-600 dark:hover:text-brand-400 transition truncate
                          ${item.level === 3 ? 'pl-3 text-slate-500 dark:text-slate-400' : 'text-slate-700 dark:text-slate-300 font-medium'}
                        `}
                        title={item.text}
                      >
                        {item.text}
