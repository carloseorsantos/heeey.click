  ...
  return (
    ...
    >
      {status === 'loading' ? (
        ...
        <p className="py-8 text-center text-sm text-slate-600 dark:text-slate-400">
          Ainda não há versões salvas. Elas aparecem aqui conforme o quadro é editado.
        </p>
      ) : (
        <div className="space-y-3">
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {version.reason === 'before_restore' ? 'Antes de uma restauração · ' : ''}
                    {version.element_count} {version.element_count === 1 ? 'elemento' : 'elementos'} ·{' '}
                    {formatDateRelative(version.created_at).toLowerCase()}
                  </p>
                </div>
                ...
                {confirmId === version.id ? (
                  ...
                  <button
                    ...
                    aria-label={`Restaurar versão de ${formatVersionDate(version.created_at)}`}
                  >
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Ao restaurar, o estado atual é salvo no histórico antes, então dá para voltar atrás.
          </p>
        </div>
      )}
    </Modal>
